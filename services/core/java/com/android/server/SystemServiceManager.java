...
public class SystemServiceManager {
     ...
     */
    @SuppressWarnings("unchecked")
    public HwSystemService startHwService(String className) {
        ...
        return startHwService(serviceClass);
    }
	...
	public void startHwService(@NonNull final HwSystemService hwService) {
        // Register it.
        mHwServices.add(hwService);
        // Start it.
        long time = SystemClock.elapsedRealtime();
        try {
            hwService.onStart();
        } catch (RuntimeException ex) {
            throw new RuntimeException("Failed to start hardware service " + hwService.getClass().getName()
                    + ": onStart threw an exception", ex);
        }
        warnIfTooLong(SystemClock.elapsedRealtime() - time, hwService, "onStart");
    }
