...
public class KeyHandler implements DeviceKeyHandler {
    ...
    private final BroadcastReceiver mUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            int[] keycodes = intent.getIntArrayExtra(
                    OffscreenGestureConstants.UPDATE_EXTRA_KEYCODE_MAPPING);
            int[] actions = intent.getIntArrayExtra(
                    OffscreenGestureConstants.UPDATE_EXTRA_ACTION_MAPPING);
    ...
    public KeyHandler(final Context context) {
        ...
        mGestureWakeLock = mPowerManager.newWakeLock(
                PowerManager.PARTIAL_WAKE_LOCK, "GestureWakeLock");
        ...
        mContext.registerReceiver(mUpdateReceiver,
                new IntentFilter(OffscreenGestureConstants.UPDATE_PREFS_ACTION));
    }
    ...
    public boolean handleKeyEvent(final KeyEvent event) {
        ...
        if (action != 0 && !mEventHandler.hasMessages(GESTURE_REQUEST)) {
            ...
            final boolean proxWakeEnabled = Settings.System.getInt(mContext.getContentResolver(),
    ...
    private class EventHandler extends Handler {
        ...
        public void handleMessage(final Message msg) {
            switch (msg.arg1) {
                case OffscreenGestureConstants.ACTION_CAMERA:
                    launchCamera();
                    break;
                case OffscreenGestureConstants.ACTION_FLASHLIGHT:
                    toggleFlashlight();
                    break;
                case OffscreenGestureConstants.ACTION_BROWSER:
                    launchBrowser();
                    break;
                case OffscreenGestureConstants.ACTION_DIALER:
                    launchDialer();
                    break;
                case OffscreenGestureConstants.ACTION_EMAIL:
                    launchEmail();
                    break;
                case OffscreenGestureConstants.ACTION_MESSAGES:
                    launchMessages();
                    break;
                case OffscreenGestureConstants.ACTION_PLAY_PAUSE_MUSIC:
                    playPauseMusic();
                    break;
                case OffscreenGestureConstants.ACTION_PREVIOUS_TRACK:
                    previousTrack();
                    break;
                case OffscreenGestureConstants.ACTION_NEXT_TRACK:
                    nextTrack();
                    break;
            }
        }
    ...
    private void doHapticFeedback() {
        ...
        if (audioManager.getRingerMode() != AudioManager.RINGER_MODE_SILENT) {
            final boolean enabled = Settings.System.getInt(mContext.getContentResolver(),
                    Settings.System.OFFSCREEN_GESTURE_HAPTIC_FEEDBACK, 1) != 0;
            if (enabled) {
