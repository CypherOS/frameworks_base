...
public class ColorManagerHelper {
    ...
    public static boolean isUsingDarkTheme(IOverlayManager om, NotificationLockscreenUserManager lockUserManager) {
        ...
        try {
            themeInfo = om.getOverlayInfo(DARK_THEME,
    ...
    public static boolean isUsingBlackTheme(IOverlayManager om, NotificationLockscreenUserManager lockUserManager) {
        OverlayInfo themeInfo = null;
        try {
            themeInfo = om.getOverlayInfo(BLACK_THEME,
                    lockUserManager.getCurrentUserId());
        } catch (RemoteException e) {
            e.printStackTrace();
        }
	...
	public static void restoreDefaultTheme(IOverlayManager om, NotificationLockscreenUserManager lockUserManager) {
		...
		for (int i = 1; i < DARK_THEME.length; i++) {
            ...
            try {
                om.setEnabled(darkTheme, false, lockUserManager.getCurrentUserId());
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
	...
	public static void updateTheme(IOverlayManager om, NotificationLockscreenUserManager lockUserManager, int systemTheme) throws RemoteException {
		switch (systemTheme) {
			case 1:
                restoreDefaultTheme(om, lockUserManager);
                break;
			case 2:
                om.setEnabled(DARK_THEME, true, lockUserManager.getCurrentUserId());
                break;
			case 3:
                om.setEnabled(BLACK_THEME, true, lockUserManager.getCurrentUserId());
                break;
		}
	}
