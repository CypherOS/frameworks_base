...
public class AmbientPlayManager implements IACRCloudListener {
    ...
    public AmbientPlayManager(Context context, StatusBar statusBar) {
        mContext = context;
            ...
            if (mResultCode == 0) {
                ...
                if (metadata.has("music")) {
                    ...
                    for (int t = 0; t < artists.length(); t++) {
						...
						((AmbientIndicationContainer) mStatusBar.getAmbientIndicationContainer()).setAmbientMusic(mSong, mArtist);
                        mLastUpdated = System.currentTimeMillis();
                        NO_MATCH_COUNT = 0;
                        mHandler.postDelayed(() -> {
							((AmbientIndicationContainer) mStatusBar.getAmbientIndicationContainer()).hideAmbientMusic();
    ...
    private class AmbientPlaySettingsObserver extends ContentObserver {
        ...
        public void updateEnabled() {
            mIsEnabled = Settings.System.getIntForUser(mContext.getContentResolver(),
                    AMBIENT_RECOGNITION, 1, UserHandle.USER_CURRENT) != 0;
            setEnabled(mIsEnabled);
			((AmbientIndicationContainer) mStatusBar.getAmbientIndicationContainer()).setEnabled(mIsEnabled);
			mHandler.post(() -> {
				((AmbientIndicationContainer) mStatusBar.getAmbientIndicationContainer()).setEnabled(mIsEnabled);
            });
        }
        ...
        public void updateAllowedOnKeyguard() {
			...
			mHandler.post(() -> {
				((AmbientIndicationContainer) mStatusBar.getAmbientIndicationContainer()).setAllowedOnKeyguard(allowedOnKeyguard);
            });
        }
    }
}
