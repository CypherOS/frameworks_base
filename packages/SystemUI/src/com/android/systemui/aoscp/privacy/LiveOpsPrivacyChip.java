...
public final class LiveOpsPrivacyChip extends LinearLayout {
	...
	private boolean mIsTypeCamera = false;
	private boolean mIsTypeMicrophone = false;
    private LinearLayout mIconsContainer;
    private TextView mText;

	private PrivacyDialogBuilder mBuilder;
	private List<PrivacyItem> mPrivacyList;
	private ArrayList mPrivacyItems;
    ...
    public void setPrivacyList(List<PrivacyItem> privacyList) {
		synchronized (mPrivacyList) {
			mPrivacyList = privacyList;
        }
		updatePrivacyItems(mPrivacyList);
	...
	private void updatePrivacyItems(List<PrivacyItem> list) {
        ...
        for (PrivacyItem item : list) {
            ...
            if (type == 1) {
                ...
                if (type == 3) {
                    mIsTypeMicrophone = true;
                }
            }
			opPackageName = item.getPackageName();
			mPrivacyItems.add(item.getPrivacyType());
        }

		if (mIconsContainer != null) {
			updateChipIcon(mBuilder, mIconsContainer);
			if (mText != null) {
				mText.setVisibility(getLiveOpCount() != 0
				        ? View.VISIBLE : View.GONE);
				mText.setText(getContext().getResources().getQuantityString(
						R.plurals.ongoing_privacy_chip_multiple_apps, getLiveOpCount(), 
						new Object[]{Integer.valueOf(getLiveOpCount())}));
			}
		}
		requestLayout();
    }

	private int getLiveOpCount() {
		if (mIsTypeCamera && mIsTypeMicrophone) {
			return 2;
		} else if (mIsTypeCamera) {
			return 1;
		} else if (mIsTypeMicrophone) {
			return 1;
		}
		return 0;
	}
        ...
        if (mIconsContainer != null) {
			...
			if (mText != null) {
                ...
                if (mBuilder.getTypes().size() == 1) {
                    ...
                    if (mText != null) {
                        mText.setText(getContext().getResources().getQuantityString(
						        R.plurals.ongoing_privacy_chip_multiple_apps, mPrivacyItems.size(), 
								new Object[]{Integer.valueOf(mPrivacyItems.size())}));
                    }
                }
            }
        }
	...
	public List<Drawable> generateIcons() {
		...
		for (PrivacyItem item : mPrivacyList) {
			icons.add(item.getPrivacyType().getIcon(getContext()));
        }
		return icons;
    }
}
