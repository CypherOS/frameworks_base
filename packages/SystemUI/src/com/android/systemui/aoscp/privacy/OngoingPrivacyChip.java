...
public final class OngoingPrivacyChip extends LinearLayout {
	...
	public void updateChipIcon(ViewGroup container) {
		...
		if (mAppOpCode == AppOpsManager.OP_COARSE_LOCATION 
			...
			imageRes = getContext().getResources().getDrawable(R.drawable.stat_sys_location);
		} else if (mAppOpCode == AppOpsManager.OP_CAMERA) {
			imageRes = getContext().getResources().getDrawable(R.drawable.stat_sys_camera);
		} else if (mAppOpCode == AppOpsManager.OP_RECORD_AUDIO) {
			imageRes = getContext().getResources().getDrawable(R.drawable.stat_sys_mic_none);
		}
