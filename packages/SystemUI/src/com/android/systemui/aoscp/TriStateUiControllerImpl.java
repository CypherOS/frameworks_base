...
public class TriStateUiControllerImpl implements ConfigurationListener, TriStateUiController, Callback {
        ...
        if (mContext != null) {
            ...
            if (res != null) {
                ...
                switch (mTriStateMode) {
                    case MODE_SILENT:
                        iconId = R.drawable.tri_state_ic_silence;
                        textId = R.string.tri_state_slient;
                        break;
                    case MODE_VIBRATE:
                        iconId = R.drawable.tri_state_ic_vibrate;
                        textId = R.string.tri_state_vibrate;
                        break;
                    case MODE_NORMAL:
                        iconId = R.drawable.tri_state_ic_ring;
                        textId = R.string.tri_state_ring;
                        break;
                }
                //int position = res.getInteger(R.integer.oneplus_config_threekey_type);
                boolean isVolLocationRight = true;
                /*if (position == TRI_STATE_UI_POSITION_LEFT) {
                    isVolLocationRight = false;
                } else if (position == TRI_STATE_UI_POSITION_RIGHT) {
                    isVolLocationRight = true;
                }*/
                switch (mOrientationType) {
                    case ROTATION_90:
                        if (isVolLocationRight) {
                            gravity = 51;
                        } else {
                            gravity = 83;
                        }
    ...
    private void updateTheme(boolean force) {
        ...
        int accent = Settings.Secure.getInt(mContext.getContentResolver(), Settings.Secure.SYSTEM_ACCENT, 0);
