...
public class SmartSpaceController implements Dumpable {
    ...
    class SmartSpaceReceiver extends BroadcastReceiver {
		...
		private SmartSpaceController mController;

        SmartSpaceReceiver(SmartSpaceController controller) {
			mController = controller;
        }

        public void onReceive(Context context, Intent intent) {
            mController.onGsaChanged();
        }
    }
    ...
    private class UserSwitchReceiver extends BroadcastReceiver {
        ...
        UserSwitchReceiver(SmartSpaceController controller, SmartSpaceReceiver receiver) {
			mController = controller;
            this();
        }

        public void onReceive(Context context, Intent intent) {
            if (mController.DEBUG) {
            ...
            if (intent.getAction().equals("android.intent.action.USER_SWITCHED")) {
                mController.mCurrentUserId = intent.getIntExtra("android.intent.extra.user_handle", -1);
                mController.mData.clear();
                mController.onExpire(true);
            }
            mController.onExpire(true);
        }
    }
    ...
    private SmartSpaceController(Context context) {
        ...
        if (!isSmartSpaceDisabledByExperiments()) {
            reloadData();
            onGsaChanged();
            context.registerReceiver(new SmartSpaceReceiver(this), GSAIntents.getGsaPackageFilter("android.intent.action.PACKAGE_ADDED", "android.intent.action.PACKAGE_CHANGED", "android.intent.action.PACKAGE_REMOVED", "android.intent.action.PACKAGE_DATA_CLEARED"));
    ...
    private SmartSpaceCard loadSmartSpaceData(boolean primary) {
        ...
        if (protoStore.load(stringBuilder.toString(), output)) {
            return SmartSpaceCard.fromWrapper(mContext, output, primary);
        }
        return null;
    }
    ...
    public void onExpire(boolean forceExpire) {
        Assert.isMainThread();
        mAlarmRegistered = false;
