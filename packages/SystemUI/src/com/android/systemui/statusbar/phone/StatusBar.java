...
public class StatusBar extends SystemUI implements DemoMode,
    ...
    protected void updateTheme() {
        ...
        if (isUsingDarkTheme() != useDarkTheme) {
            final boolean setDarkTheme = useDarkTheme;
            for (String darkTheme: ColorManagerHelper.DARK_THEME) {
                mUiOffloadThread.submit(() -> {
                    try {
                        mOverlayManager.setEnabled(darkTheme, setDarkTheme, mLockscreenUserManager.getCurrentUserId());
        ...
        if (isUsingBlackTheme() != useBlackTheme) {
            ...
            for (String blackTheme: ColorManagerHelper.BLACK_THEME) {
                mUiOffloadThread.submit(() -> {
                    try {
                        mOverlayManager.setEnabled(blackTheme, setBlackTheme, mLockscreenUserManager.getCurrentUserId());
                        Log.d(TAG, "Successfully changed theme", e);
                    } catch (RemoteException e) {
                        Log.d(TAG, "Can't change theme", e);
                    }
