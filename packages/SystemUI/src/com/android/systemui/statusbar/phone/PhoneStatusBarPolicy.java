...
public class PhoneStatusBarPolicy implements Callback, Callbacks,
    ...
    private final LiveOpsController.Callback mPrivacyCallback = new LiveOpsController.Callback() {
        ...
        public void onPrivacyChanged(List<LiveOpItem> activeOps) {
			updateLiveOpItems(activeOps);
        }
    };

	private void updateLiveOpItems(List<LiveOpItem> activeOps) {
        boolean isTypeCamera = false;
        boolean isTypeMicrophone = false;
		int locationOpType = AppOpsManager.OP_COARSE_LOCATION || AppOpsManager.OP_FINE_LOCATION 
		        || AppOpsManager.OP_MONITOR_LOCATION || AppOpsManager.OP_MONITOR_HIGH_POWER_LOCATION;
        for (LiveOpItem item : activeOps) {
			int type = item.getOp();
            if (type == AppOpsManager.OP_CAMERA) {
                isTypeCamera = true;
            } else if (type != locationOpType) {
