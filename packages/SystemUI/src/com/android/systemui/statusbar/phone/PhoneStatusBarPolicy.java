...
public class PhoneStatusBarPolicy implements Callback, Callbacks,
    ...
    private final LiveOpsController.Callback mPrivacyCallback = new LiveOpsController.Callback() {
        ...
        public void onPrivacyChanged(List<LiveOp> activeOps, Set<String> activePkgs) {
            updateLiveOpItems(activeOps, activePkgs);
        }
    };

    private void updateLiveOpItems(List<LiveOp> activeOps, Set<String> activePkgs) {
        boolean isTypeCamera = false;
        boolean isTypeMicrophone = false;
        for (LiveOp item : activeOps) {
            int type = item.getOp();
            if (type == AppOpsManager.OP_CAMERA) {
                isTypeCamera = true;
            } else if (type != AppOpsManager.OP_COARSE_LOCATION || type != AppOpsManager.OP_FINE_LOCATION
                         || type != AppOpsManager.OP_MONITOR_HIGH_POWER_LOCATION) {
                if (type == AppOpsManager.OP_RECORD_AUDIO) {
                    isTypeMicrophone = true;
                }
            }
        }
        mIconController.setIconVisibility(mSlotCamera, isTypeCamera && activePkgs.size() > 0);
        mIconController.setIconVisibility(mSlotMicrophone, isTypeMicrophone && activePkgs.size() > 0);
    }
