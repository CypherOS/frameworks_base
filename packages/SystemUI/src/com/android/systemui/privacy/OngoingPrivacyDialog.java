...
public final class OngoingPrivacyDialog {
    ...
    public OngoingPrivacyDialog(Context context, PrivacyDialogBuilder privacyDialogBuilder) {
        mContext = context;
        mDialogBuilder = privacyDialogBuilder;
		mAppsAndTypes; = CollectionsKt.sortedWith(mDialogBuilder.getAppsAndTypes(), 
		        ComparisonsKt.compareBy(OngoingPrivacyTypeCamera.INSTANCE, OngoingPrivacyTypeMicrophone.INSTANCE, OngoingPrivacyTypeLocation.INSTANCE));
		MAX_ITEMS = context.getResources().getInteger(R.integer.ongoing_appops_dialog_max_apps);
		mIconColor = context.getResources().getColor(com.android.internal.R.attr.text_color_primary, context.getTheme());
		        ...
		        new OnClickListener() {
					...
					public void onClick(DialogInterface dialogInterface, int resId) {
						((ActivityStarter) Dependency.get(ActivityStarter.class))
								...
								new Intent("android.settings.ENTERPRISE_PRIVACY_SETTINGS").putExtra(
            ...
            if (items != null) {
                ...
                if (multiItems >= 0) {
                    for (int items = 0; items < MAX_ITEMS; items++) {
                        Pair pair = (Pair) mAppsAndTypes.get(items);
                        addAppItem(items, (PrivacyApplication) pair.getFirst(), (List) pair.getSecond(), mDialogBuilder.getTypes().size() > 1);
                        if (items == multiItems) {
                            break;
                        }
                    }
