...
public class PrivacyDialogBuilder {
    ...
    public PrivacyDialogBuilder(Context context, List<PrivacyItem> list) {
        ...
        mAppsAndTypes = Collection.sortedWith(toList, Comparator.compareBy(topRank));
        ArrayList privacyTypes = new ArrayList(Iterable.collectionSizeOrDefault(list, 10));
        for (PrivacyItem privacyType : list) {
            privacyTypes.add(privacyType.getPrivacyType());
        }
        mTypes = Collection.sorted(Collection.distinct(privacyTypes));
    ...
    public List<Drawable> generateIconsForApp(List<? extends PrivacyType> list) {
        List<PrivacyType> sorted = Collection.sorted(list);
        ArrayList generatedForApp = new ArrayList(Iterable.collectionSizeOrDefault(sorted, 10));
    ...
    public List<Drawable> generateIcons() {
        ArrayList generated = new ArrayList(Iterable.collectionSizeOrDefault(mTypes, 10));
    ...
    private <T> StringBuilder joinWithAnd(List<? extends T> list) {
        ...
        Collection.joinTo(subList, sbAppend, mSeparator, null, null, 0, null, null, com.android.internal.R.styleable.windowNoTitle, null);
        StringBuilder sb = (StringBuilder) sbAppend;
        sb.append(mLastSeparator);
        sb.append(Collection.last(list));
        return sb;
    }
    ...
    public String joinTypes() {
        ...
        if (mTypes.size() != 1) {
            ...
            ArrayList privacyName = new ArrayList(Iterable.collectionSizeOrDefault(list, 10));
