...
public class PrivacyItemController implements Callback {
    ...
    private static final List<String> mIntents = Collections.listOf("android.intent.action.USER_FOREGROUND", "android.intent.action.MANAGED_PROFILE_ADDED", "android.intent.action.MANAGED_PROFILE_REMOVED");
    ...
    private List<Integer> mCurrentUserIds = java.util.Collections.emptyList();
    private boolean mListening;
    private final Runnable mNotifyChanges;
    private List<PrivacyItem> mPrivacyList = java.util.Collections.emptyList();
    ...
    public class Receiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (Collections.contains(mPrivacyItemController.mCompanion.getIntents(), intent != null ? intent.getAction() : null)) {
    ...
    private void update(boolean fromReceiver) {
        if (fromReceiver) {
            ...
            ArrayList ids = new ArrayList(Collections.collectionSizeOrDefault(profiles, 10));
    ...
    private void updatePrivacyList() {
        ...
        for (Integer userIds : mCurrentUserIds) {
            Collections.addAll(opItems, mAppOpsController.getActiveAppOpsForUser(userIds.intValue()));
        }
