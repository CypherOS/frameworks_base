...
public class PrivacyItemController implements Callback {
    ...
    public class Receiver extends BroadcastReceiver {
        public void onReceive(Context context, Intent intent) {
            if (CollectionsKt.contains(PrivacyItemController.mCompanion.getIntents(), intent != null ? intent.getAction() : null)) {
    ...
    public PrivacyItemController(Context context) {
        ...
        mUserSwitcherReceiver = new Receiver();
    ...
    private void update(boolean fromReceiver) {
        if (fromReceiver) {
            ...
            ArrayList ids = new ArrayList(IterablesKt.collectionSizeOrDefault(profiles, 10));
    ...
    private void removeCallback(WeakReference<Callback> weakReference) {
        mCallbacks.removeIf(mCallbacks != null && mCallbacks.equals(mCallbacks.get()));
    ...
    private void updatePrivacyList() {
        List<Number> list = mCurrentUserIds;
        ArrayList<AppOpItem> opItems = new ArrayList();
        for (Number intValue : list) {
            CollectionsKt__MutableCollectionsKt.addAll(opItems, mAppOpsController.getActiveAppOpsForUser(intValue.intValue()));
        }
