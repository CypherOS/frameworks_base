...
public class EnhancedEstimatesImpl implements EnhancedEstimates {
    ...
    private static final String BATTERY_ESTIMATE_BASED_ON_USAGE_COL = "is_based_on_usage";
    private static final String BATTERY_ESTIMATE_COL = "battery_estimate";
            ...
            if (query != null) {
                try {
                    if (query.moveToFirst()) {
                        ...
                        if (query.getColumnIndex(BATTERY_ESTIMATE_BASED_ON_USAGE_COL) != -1) {
                            if (query.getInt(query.getColumnIndex(BATTERY_ESTIMATE_BASED_ON_USAGE_COL)) == 0) {
                                isBasedOnUsage = false;
                            }
                        }
                        Estimate estimate = new Estimate(query.getLong(query.getColumnIndex(BATTERY_ESTIMATE_COL)), isBasedOnUsage);
                        if (query != null) {
                            query.close();
                        }
                        return estimate;
