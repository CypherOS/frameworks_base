...
public class AppOpsControllerImpl implements AppOpsController, OnOpActiveChangedListener, OnOpNotedListener, Dumpable {
    ...
    @Override
    public void addCallback(int[] keys, Callback callback) {
        int length = keys.length;
        int code = 0;
        boolean hasCode = false;
        while (code < length) {
            if (mCallbacksByCode.containsKey(Integer.valueOf(keys[code]))) {
                ((Set) mCallbacksByCode.get(Integer.valueOf(keys[code]))).add(callback);
                hasCode = true;
            }
            code++;
        }
